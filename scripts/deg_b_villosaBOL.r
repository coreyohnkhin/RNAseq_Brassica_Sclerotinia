...
ctrl_bol <- read_delim(
  "data/featurecountmatrices/N1896Pet_ControlBOL.txt",
  skip   = 1,
  show_col_types = FALSE
)

inf_bol  <- read_delim(
  "data/featurecountmatrices/N1896Pet_InfectedBOL.txt",
  skip   = 1,
  show_col_types = FALSE
)

drop_annot <- c("Chr", "Start", "End", "Strand", "Length")
counts_ctrl <- ctrl_bol |> select(-all_of(drop_annot))
counts_inf  <- inf_bol  |> select(-all_of(drop_annot))
...
pdf("figures/heatmap_top50_BOL.pdf", width = 6, height = 8)
draw(ht)
dev.off()

# Outputs:
# results/results_BOL_infected_vs_control.csv
# figures/heatmap_top50_BOL.pdf
